...
#if 0
void recognize_helper_old(const Mat& image){
...
char recognize_char(Mat& charInputImage){
	
	float bestScore=0;
	char bestChar='i'; // default to 'i'
   
   for (char c = 'a'; c <= 'z'; ++c){
      
      Mat charTemplateImage = get_char_image(c);
	
//	for (int j=0; j < letterImages.size() ; ++j){
	//	Mat imlt = letterImages[j];		
		
		Size ch = charTemplateImage.size();
		Size lt = charInputImage.size();
...
//		}
		...
		if (aspect_ratio_measure>0.5){
         ...
         double score = match_image(charInputImage, charTemplateImage);
...
string 
recognize_word(const Mat& inputImage, const WordRect& wordRect){
   
   string result;
   for (int i = 0; i < wordRect.charRects.size(); ++i){
      Rect charRect = wordRect.charRects[i];
      Mat charImage(inputImage, charRect);
   
      char bestChar = recognize_char(charImage);
      

      result += bestChar;
      //imshowDebugZoom("t",charImage);      
      //dout << bestChar;
   }
...
recognize_words(const Mat& inputImageColor){
   return recognize_helper(inputImageColor);
}